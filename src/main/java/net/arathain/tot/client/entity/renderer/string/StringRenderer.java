...
import static net.arathain.tot.common.util.StringUtils.*;

/**
 * @author legoatoom
 * **/
public class StringRenderer {
    private final Object2ObjectOpenHashMap<BakeKey, StringModel> models = new Object2ObjectOpenHashMap<>(256);
    ...
    public static class BakeKey {
        ...
        public BakeKey(Vec3d srcPos, Vec3d dstPos) {
            ...
            float dXZ = StringUtils.distanceBetween(
    ...
    public void render(VertexConsumer buffer, MatrixStack matrices, Vec3f chainVec, int blockLight0, int blockLight1, int skyLight0, int skyLight1) {
        StringModel model = buildModel(chainVec);
        model.render(buffer, matrices, blockLight0, blockLight1, skyLight0, skyLight1);
    }

    private StringModel buildModel(Vec3f chainVec) {
        float desiredSegmentLength = 1f / 2;
        int initialCapacity = (int) (2f * StringUtils.lengthOf(chainVec) / desiredSegmentLength);
        StringModel.Builder builder = StringModel.builder(initialCapacity);
    ...
    private void buildFaceVertical(StringModel.Builder builder, Vec3f v, float angle, int uvu) {
        float actualSegmentLength = 1f / 2;
    ...
    private void buildFace(StringModel.Builder builder, Vec3f v, float angle, int uvu) {
        float actualSegmentLength, desiredSegmentLength = 1f / 2;
        float distance = StringUtils.lengthOf(v), distanceXZ = (float) Math.sqrt(v.getX()*v.getX() + v.getZ()*v.getZ());
        ...
        actualSegmentLength = StringUtils.distanceBetween(point0, point1);
