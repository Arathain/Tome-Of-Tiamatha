...
public class DriderDenGenerator {
    public static Optional<StructurePiecesGenerator<StructurePoolFeatureConfig>> generate(StructurePiecesGeneratorFactory.Context<StructurePoolFeatureConfig> inContext, PieceFactory pieceFactory, BlockPos pos) {
        ...
        if (!biomePredicate.test(chunkGenerator.method_16359(BiomeCoords.fromBlock(centerX), BiomeCoords.fromBlock(y), BiomeCoords.fromBlock(centerZ))))
            return Optional.empty();
        ...
        return Optional.of((structurePiecesCollector, context) -> {
            ...
            DriderDenPoolGenerator structurePoolGenerator = new DriderDenPoolGenerator(registry, 10, pieceFactory, chunkGenerator, structureManager, list, chunkRandom);
            structurePoolGenerator.structurePieces.addLast(new DriderDenPoolStructurePiece(poolStructurePiece, new MutableObject<>(VoxelShapes.combineAndSimplify(VoxelShapes.cuboid(box), VoxelShapes.cuboid(Box.from(pieceBoundingBox)), BooleanBiFunction.ONLY_FIRST)), 0, null));

            // Go through all structure pieces in the project.
            while (!structurePoolGenerator.structurePieces.isEmpty()) {
                DriderDenPoolStructurePiece shapedPoolStructurePiece = structurePoolGenerator.structurePieces.removeFirst();
    ...
    static final class DriderDenPoolGenerator {
        ...
        final Random random;
        final Deque<DriderDenPoolStructurePiece> structurePieces = Queues.newArrayDeque();
        ...
        static final HashSet<Identifier> terrainCheckIgnoredPools = new HashSet<>(Arrays.asList(
                new Identifier(TomeOfTiamatha.MODID, "bee"),
                new Identifier(TomeOfTiamatha.MODID, "deco_blocks"),
                new Identifier(TomeOfTiamatha.MODID, "deco_coverings"),
                new Identifier(TomeOfTiamatha.MODID, "deco_wallpapers"),
                new Identifier(TomeOfTiamatha.MODID, "iron_golem"),
                new Identifier(TomeOfTiamatha.MODID, "villagers"),
                new Identifier(TomeOfTiamatha.MODID, "armor_stands")
        ));

        DriderDenPoolGenerator(Registry<StructurePool> registry, int maxSize, PieceFactory pieceFactory, ChunkGenerator chunkGenerator, StructureManager structureManager, List<? super PoolStructurePiece> children, Random random) {
            ...
            this.random = random;
            fallback_down = registry.get(new Identifier(TomeOfTiamatha.MODID, "den_fallback_down_pool"));
            fallback_side = registry.get(new Identifier(TomeOfTiamatha.MODID, "den_fallback_side_pool"));
            end_cap = registry.get(new Identifier(TomeOfTiamatha.MODID, "den_end"));
        }
        ...
        boolean tryPlacePiece(PoolStructurePiece piece, int currentSize, HeightLimitView world, int boundsMinY, Structure.StructureBlockInfo structureBlock, MutableObject<VoxelShape> structureShape, Direction structureBlockFaceDirection, BlockPos structureBlockPosition, BlockPos structureBlockAimPosition, StructurePoolElement element, boolean doTerrainCheck) {
            ...
            for (BlockRotation randomizedRotation : BlockRotation.randomRotationOrder(this.random)) {
                ...
                for (Structure.StructureBlockInfo structureBlockInfo : structureBlocksInStructure) {
                    ...
                    // STONEHOLM CUSTOM: Skip if top of bounding box is above terrain. This is extremely hacky. Like, genuinely this is terrible. And I, Arathain, have copied it.
                    ...
                    if (currentSize + 1 <= this.maxSize) // Whilst this is not the end.
                        this.structurePieces.addLast(new DriderDenPoolStructurePiece(poolStructurePiece, structureShape, currentSize + 1, structureBlockPos));
    ...
    record DriderDenPoolStructurePiece(PoolStructurePiece piece, MutableObject<VoxelShape> pieceShape, int currentSize, BlockPos sourceBlockPos) {}
}
